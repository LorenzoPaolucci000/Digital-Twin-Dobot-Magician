...
def start_server_socket(host='localhost', port=12345):
    def server_thread():
        ...
        try:
            while True:
                ...
                if joint_rotations:
                    data_to_send = ','.join([f"{angle:.2f}" for angle in joint_rotations]) # 2 cifre decimali
...
#             client_socket.close()
#             server_socket.close()
#         bpy.app.timers.register(start_server_socket, first_interval=1.0)
#         server_socket.setblocking(False)
#
# # Avviare il server socket in modo non bloccante
# bpy.app.timers.register(start_server_socket)
